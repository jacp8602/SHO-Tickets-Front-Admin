...
export class PromocodesListComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.searchForm
      .get("keyword")
      ?.valueChanges.pipe(takeUntil(this._unsubscribeAll), debounceTime(300))
      .subscribe(() => {
        this.pageIndex = 1;
        this._loadPromocodes();
      });
  ...
  private _loadPromocodes(): void {
    ...
    this._promocodesService
      .getPromocodes(apiPage, this.pageSize, filters)
      .pipe(
  ...
  deletePromocode(promocode: PromoCode): void {
    if (
      confirm(`Are you sure you want to delete ${promocode.promoCodeName}?`)
    ) {
      this.isLoading = true;
  ...
  toggleActive(promocode: PromoCode, event: any): void {
    ...
    this._promocodesService.updatePromocode(promocode.id, updated).subscribe({
      next: () => {
        this._loadPromocodes();
      },
      error: (error) => {
