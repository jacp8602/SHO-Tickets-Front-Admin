...
export class AuthVerificationCodeComponent implements OnInit, AfterViewInit {
  ...
  async initializeRecaptcha(): Promise<void> {
    ...
    if (this._firebase2FAService.getRecaptchaVerifier()) {
      console.log("[VerificationCode] reCAPTCHA verifier already initialized");
      return;
    }
    ...
    console.log(
      "[VerificationCode] reCAPTCHA verifier created, waiting for render...",
    );

    // Wait for reCAPTCHA to be fully rendered and ready
    try {
      const verifier = this._firebase2FAService.getRecaptchaVerifier();
      if (!verifier) {
  ...
  start2faFlow(): void {
    ...
    if (!recaptchaVerifier) {
      console.error(
        "[VerificationCode] reCAPTCHA verifier not initialized - this should not happen!",
      );
      this.alert = {
        type: "error",
  ...
  sendSmsForEnrollment(): void {
    ...
    if (!formattedPhoneNumber) {
      this.alert = {
        type: "error",
        message:
          "Invalid phone number format. Please include country code (e.g., +1 for USA)",
      };
      this.showAlert = true;
      return;
    }
    ...
    this._firebase2FAService
      ...
      .subscribe({
        next: (verificationId) => {
          console.log(
            "[VerificationCode] SMS sent successfully. Verification ID:",
            verificationId,
          );
  ...
  verifyCode(): void {
    ...
    if (this.isEnrolling && this.verificationId && this.userData?.user) {
      // Complete enrollment
      this._firebase2FAService
        .confirmEnrollMfa(this.userData.user, this.verificationId, smsCode)
        .pipe(
          finalize(() => {
            this.verifyForm.enable();
          }),
  ...
  resendCode(): void {
    ...
    if (this.isEnrolling && this.userData?.user) {
      ...
      if (!recaptchaVerifier) {
        console.error(
          "[VerificationCode] reCAPTCHA verifier not found for resend",
        );
        this.alert = {
          type: "error",
          message: "reCAPTCHA not initialized. Please refresh and try again.",
